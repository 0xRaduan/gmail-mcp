...
```bash
# Install globally
bun install -g @thesummarycompany/gmail-mcp
...
The evals package loads an mcp client that then runs the index.ts file, so there is no need to rebuild between tests. You can load environment variables by prefixing the command. Full documentation can be found [here](https://www.mcpevals.io/docs).
